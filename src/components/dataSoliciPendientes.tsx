...
import {DATASOLIPENDIENTES} from '../environment'

function DataSoliPendientes() {
  

  const [datos, setDatos] = useState<any>(null);

  async function obtenerDatos() {
    const respuesta = await fetch(DATASOLIPENDIENTES);
  ...
  return (
    <div className='centrar'>
        SOY DATA SOLICITUDES PENDIENTES
      {datos ? (
        <div >
          {datos.map((dato: any) => (
            <div key={dato.c[1].v} >
...
export default DataSoliPendientes;
